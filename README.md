...
1. Đăng ký / Đăng nhập:
   - [x] Đăng ký tài khoản mới thành công, mật khẩu được mã hoá bằng bcrypt.
   - [x] Đăng nhập thành công, hệ thống trả về JWT token, userId, username, role.
   - [x] Đăng nhập sai mật khẩu → hệ thống trả về lỗi "fail".
   - [x] Đăng ký trùng username → hiển thị thông báo lỗi rõ ràng cho người dùng.

2. Xem menu & Tìm kiếm:
   - [x] Trang chủ hiển thị danh sách món ăn từ API (không cần đăng nhập).
   - [x] Lọc món ăn theo danh mục hoạt động đúng.
   - [x] Các món bị xóa mềm (daXoa = True) không hiển thị trên menu.
   - [x] Hiển thị đúng trạng thái hết hàng (conHang = False).

3. Giỏ hàng:
   - [x] Thêm món vào giỏ hàng khi đã đăng nhập.
   - [x] Xoá món khỏi giỏ hàng.
   - [x] Tính đúng tổng tiền tạm tính.
   - [x] Badge số lượng trên icon giỏ hàng cập nhật theo thời gian thực.

4. Đặt hàng & Thanh toán:
   - [x] Tạo đơn hàng mới từ giỏ hàng, trạng thái ban đầu là "pending".
   - [x] Sao chép đúng thông tin giá tại thời điểm đặt vào ChiTietDonHang.
   - [x] Giỏ hàng được xoá sau khi đặt hàng thành công.
   - [x] Hỗ trợ cả hai hình thức thanh toán: COD và online (mô phỏng).
   - [x] Chuyển hướng đến trang thành công sau khi đặt hàng.

5. Quản lý đơn hàng (Nhân viên / Quản lý):
   - [x] Dashboard hiển thị danh sách đơn hàng mới nhất.
   - [x] Cập nhật trạng thái đơn hàng tuần tự (pending → confirmed → preparing → delivering → completed).
   - [x] Hủy đơn hàng (chuyển sang "cancelled").
   - [x] Quyền truy cập được bảo vệ bằng JWT token (chỉ staff/admin mới được gọi API quản lý đơn).

6. Quản lý món ăn (Quản lý):
   - [x] Thêm món ăn mới kèm hình ảnh upload.
   - [x] Chỉnh sửa thông tin món ăn.
   - [x] Xóa mềm món ăn (daXoa = True), dữ liệu vẫn được giữ trong DB.
   - [x] Khôi phục món ăn đã xóa mềm.
   - [x] Nhân viên toggle trạng thái còn hàng / hết hàng.

7. Lịch sử đặt hàng (Khách hàng):
   - [x] Hiển thị đúng danh sách đơn hàng của khách hàng đang đăng nhập.
   - [x] Hiển thị chi tiết từng đơn hàng (món ăn, số lượng, giá tại thời điểm đặt).
   - [x] Hiển thị đúng trạng thái đơn hàng hiện tại.

Báo cáo:
- Hệ thống đã hoàn thành đầy đủ các chức năng MVP đã đề ra.
- Tất cả các API backend được kiểm thử thủ công qua giao diện người dùng React.
- Phân quyền (customer / staff / admin) hoạt động đúng theo yêu cầu nghiệp vụ.
- Soft Delete được triển khai thành công tại bảng MonAn (cột daXoa), đảm bảo tính toàn vẹn dữ liệu lịch sử đơn hàng.
...
# Mở file app/database.py và chỉnh sửa DATABASE_URL cho phù hợp:
# DATABASE_URL = "postgresql://postgres:<password>@localhost:5432/food_ordering_system"

# Tạo database và chạy script khởi tạo
# (Chạy file SQL trong PostgreSQL trước)
# psql -U postgres -f FoodOrderingSystemDB.sql
...
### 4. Tài khoản mặc định để kiểm thử

| Vai trò   | Username      | Password   | Ghi chú                         |
|-----------|---------------|------------|---------------------------------|
| Admin     | `admin`       | `admin123` | Truy cập Dashboard quản lý       |
| Khách hàng| `khangg5`     | `123`      | Test đặt món, giỏ hàng           |
| Khách hàng| `khtest_db`   | `123`      | Có sẵn 4 đơn hàng mẫu để test   |
...
```
SoucreCode/
├── food-order-backend/         # Backend API (FastAPI + SQLAlchemy)
│   ├── app/
│   │   ├── main.py             # Khởi tạo FastAPI app, đăng ký routers, CORS
│   │   ├── database.py         # Cấu hình kết nối PostgreSQL (SQLAlchemy engine)
│   │   ├── models.py           # Định nghĩa các bảng DB (ORM models)
│   │   ├── schemas.py          # Pydantic schemas dùng cho request/response
│   │   ├── auth.py             # Logic mã hoá mật khẩu (bcrypt) và JWT token
│   │   ├── deps.py             # Hàm phụ thuộc (get_db, verify_token...)
│   │   └── routers/
│   │       ├── auth.py         # API: đăng ký, đăng nhập
│   │       ├── product.py      # API: quản lý món ăn (CRUD + soft delete)
│   │       ├── cart.py         # API: thêm/xem/xóa giỏ hàng
│   │       ├── order.py        # API: đặt hàng, lịch sử, cập nhật trạng thái
│   │       └── category.py     # API: quản lý danh mục
│   ├── static/uploads/         # Thư mục lưu hình ảnh upload
│   ├── FoodOrderingSystemDB.sql # Script tạo database PostgreSQL
│   ├── setup_db.py             # Script khởi tạo DB qua SQLAlchemy
│   ├── migrate_add_daxoa.py    # Migration thêm cột daXoa vào bảng MonAn
│   └── seed_orders.py          # Script tạo dữ liệu đơn hàng mẫu
│
├── food-order-frontend/        # Frontend (React + Vite)
│   └── src/
│       ├── App.jsx             # Cấu hình routing, Navbar chính
│       ├── main.jsx            # Entry point React
│       ├── context/
│       │   └── CartContext.jsx # Quản lý state giỏ hàng (Context API)
│       └── pages/
│           ├── Home.jsx           # Trang chủ: xem menu, lọc theo danh mục
│           ├── Login.jsx          # Trang đăng nhập
│           ├── Register.jsx       # Trang đăng ký tài khoản
│           ├── Cart.jsx           # Trang giỏ hàng
│           ├── Checkout.jsx       # Trang thanh toán & điền địa chỉ giao hàng
│           ├── OrderSuccess.jsx   # Trang thông báo đặt hàng thành công
│           ├── CustomerOrders.jsx # Trang lịch sử đơn hàng (khách hàng)
...
|--------|-----------------------|-------------------------------------|------------|
| POST   | `/auth/register`      | Đăng ký tài khoản khách hàng mới   | Public     |
| POST   | `/auth/login`         | Đăng nhập, trả về JWT token         | Public     |
| GET    | `/auth/create-admin`  | Tạo tài khoản admin mặc định        | Public     |
| GET    | `/auth/accounts`      | Xem danh sách tài khoản             | Public     |

### Sản phẩm (`/products`)

| Method | Endpoint                       | Mô tả                                       | Phân quyền    |
|--------|--------------------------------|----------------------------------------------|---------------|
| GET    | `/products/`                   | Lấy danh sách món ăn đang hiển thị (menu)   | Public        |
| GET    | `/products/all`                | Lấy toàn bộ món ăn kể cả đã xóa mềm        | Public        |
| POST   | `/products/`                   | Thêm món ăn mới                              | Admin         |
| DELETE | `/products/{maMon}`            | Xóa mềm món ăn (daXoa = True)               | Admin         |
| PUT    | `/products/{maMon}/restore`    | Khôi phục món ăn đã xóa mềm                 | Admin         |
| PUT    | `/products/{maMon}/toggle-status` | Đổi trạng thái còn hàng / hết hàng       | Staff / Admin |
| POST   | `/products/upload-image`       | Upload hình ảnh món ăn                       | Admin         |

### Giỏ hàng (`/cart`)

| Method | Endpoint                              | Mô tả                          | Phân quyền |
|--------|---------------------------------------|-------------------------------|------------|
| POST   | `/cart/add?user_id={id}`              | Thêm món vào giỏ hàng         | Customer   |
| GET    | `/cart/{user_id}`                     | Xem giỏ hàng của khách hàng   | Customer   |
| DELETE | `/cart/remove/{user_id}/{product_id}` | Xóa món khỏi giỏ hàng         | Customer   |

### Đơn hàng (`/orders`)

| Method | Endpoint                        | Mô tả                                    | Phân quyền    |
|--------|---------------------------------|-------------------------------------------|---------------|
| POST   | `/orders/checkout`              | Đặt hàng từ giỏ hàng hiện tại            | Customer      |
| GET    | `/orders/`                      | Xem toàn bộ đơn hàng                     | Staff / Admin |
| GET    | `/orders/history/{user_id}`     | Xem lịch sử đơn hàng của khách hàng      | Customer      |
| PUT    | `/orders/{order_id}/status`     | Cập nhật trạng thái đơn hàng             | Staff / Admin |
...
| DELETE | `/categories/{maDanhMuc}`  | Xóa danh mục                 | Admin      |
